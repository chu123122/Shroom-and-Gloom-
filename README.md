...
注入完成后，插件还会把游戏全部中文字符**一次性预热**进字体图集。

**为什么需要**：游戏里所有中文字体的图集在序列化数据里都是 **0×0、零图像数据**
（`WDXLLubrifontSC-Regular` / `NotoSansSC-Regular` / `NotoSansSC-Bold`），
这是 TMP **Dynamic** 字体资产的特征——图集运行时才分配，用到哪个字才往图集里加。
对比拉丁文的 `ShroomScript` 是 1024×1024 完整烘焙的静态图集。

**后果**：某个中文字形在加入图集之前渲染为**空白**；而且 TMP 会**动态扩页**
（实测预热时 `WDXLLubrifontSC-Regular` 从 1 页涨到 3 页，`NotoSansSC-*` 涨到 4 页），
扩页瞬间仍在使用旧页的文本会变空白，直到被重建。
表现为卡名等文字**有时显示有时不显示**，切英文则正常（拉丁字形早已烘焙）。

**做法**：`charset.txt`（离线从 5 张中文表提取的全部 1714 个去重字符）+
启动时对每个 Dynamic 字体调用 `TMP_FontAsset.TryAddCharacters` 预热，
在界面渲染之前把字形就位、图集页数稳定下来。

日志里可以看到每个字体的图集模式、页数变化与预热结果：

```
字体 WDXLLubrifontSC-Regular: 模式=Dynamic 图集页=1 首页=0x0 多图集=True
  预热结果: 全部加入，图集页 1 → 3
```
